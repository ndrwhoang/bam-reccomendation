...
class RecommendationPipeline(Pipeline):
    ...
    def preprocess(self, text, max_length=512):
        ...
        candidate_ids = input_encoding["input_ids"][0, :].detach().clone().unsqueeze(0)
        candidate_attention_mask = input_encoding["attention_mask"][0, :].detach().clone().unsqueeze(0)
